 ...
 * ... et sont maintenant reliés par la navigation ci-dessous
 * (les trois TODO ont été complétés).
 */
...
// ---------------------------------------------------------------------------
// LA NAVIGATION — c'est ici que tout se joue (3 TODO, tous faits)
// ---------------------------------------------------------------------------
        ...
        composable("liste") {
            EcranListe(
                ...
                onProduitClick = { produitId ->
                    // TODO 2 — FAIT : naviguer vers le détail du produit cliqué.
                    navController.navigate("detail/$produitId")
                }
            )
        }

        // TODO 1 — FAIT : la route du détail, avec son argument produitId.
        composable("detail/{produitId}") { backStackEntry ->
            // On relit l'argument, transporté sous forme de texte dans l'URL
            // de la route, puis on le convertit en Int (toIntOrNull() gère
            // proprement le cas où la conversion échouerait, sans crash).
            val id = backStackEntry.arguments
                ?.getString("produitId")?.toIntOrNull()

            // On retrouve l'objet Produit complet à partir de cet id :
            // c'est bien l'identifiant qui voyage entre les écrans, pas
            // l'objet Produit lui-même (voir l'objectif du TP à ce sujet).
            val produit = produits.find { it.id == id }

            // Garde-fou : si l'id est invalide ou introuvable, on n'affiche
            // rien plutôt que de planter (null safety, comme au TP4).
            if (produit != null) {
                EcranDetail(
                    produit = produit,
                    onRetour = {
                        // TODO 3 — FAIT : revenir à la liste (dépiler).
